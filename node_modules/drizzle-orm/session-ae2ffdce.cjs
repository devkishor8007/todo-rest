...
_a$c = alias.entityKind, alias.Table.Symbol.Columns, _b$8 = InlineForeignKeys, _c$4 = alias.Table.Symbol.ExtraConfigBuilder;
...
ForeignKey[_b$7] = 'MySqlForeignKey';
...
class MySqlDialect {
    ...
    buildSelectQuery({ withList, fields, fieldsFlat, where, having, table, joins, orderBy, groupBy, limit, offset, lockingClause, distinct, }) {
        ...
        for (const [index, joinMeta] of joins.entries()) {
            ...
            if (alias.is(table, MySqlTable)) {
                ...
                joinsArray.push(alias.sql `${alias.sql.raw(joinMeta.joinType)} join ${tableSchema ? alias.sql `${alias.sql.identifier(tableSchema)}.` : undefined}${alias.sql.identifier(origTableName)}${alias$1 && alias.sql ` ${alias.sql.identifier(alias$1)}`} on ${joinMeta.on}`);
            }
                ...
                joinsArray.push(alias.sql `${alias.sql.raw(joinMeta.joinType)} join ${viewSchema ? alias.sql `${alias.sql.identifier(viewSchema)}.` : undefined}${alias.sql.identifier(origViewName)}${alias$1 && alias.sql ` ${alias.sql.identifier(alias$1)}`} on ${joinMeta.on}`);
            }
            else {
    ...
    buildRelationalQuery(fullSchema, schema, tableNamesMap, table, tableConfig, config, tableAlias, relationColumns, isRoot = false) {
        ...
        for (const { key: selectedRelationKey, value: selectedRelationValue } of selectedRelations) {
            ...
            const elseField = alias.sql `json_arrayagg(json_array(${alias.sql.join(builtRelation.selection.map(({ dbKey: key, isJson }) => {
                const field = alias.sql `${alias.sql.identifier(relationAlias)}.${alias.sql.identifier(key)}`;
                return isJson ? alias.sql `cast(${field} as json)` : field;
            }), alias.sql `, `)}))`;
            ...
            const field = alias.sql `if(count(${countSql}) = 0, '[]', ${elseField})`.as(selectedRelationKey);
        ...
        const finalFieldsFlat = isRoot
            ? [
                ...
                ...builtRelationFields.map(({ path, field }) => ({
                    path,
                    field: alias.sql `cast(${alias.sql.identifier(field.fieldAlias)} as json)`,
                })),
            ]
            : [
                ...Object.entries(tableConfig.columns).map(([tsKey, column]) => ({
        ...
        if (this._.schema) {
            for (const [tableName, columns] of Object.entries(this._.schema)) {
                this.query[tableName] =
                    new RelationalQueryBuilder(schema.fullSchema, this._.schema, this._.tableNamesMap, schema.fullSchema[tableName], columns, dialect, session);
            }
        }
    }
    $with(alias$1) {
...
class MySqlSession {
    ...
    getSetTransactionSQL(config) {
        ...
        return parts.length ? alias.sql.fromList(['set transaction ', parts.join(' ')]) : undefined;
    }
    getStartTransactionSQL(config) {
        ...
        return parts.length ? alias.sql.fromList(['start transaction ', parts.join(' ')]) : undefined;
    }
}
_b = alias.entityKind;
...
//# sourceMappingURL=session-ae2ffdce.cjs.map
